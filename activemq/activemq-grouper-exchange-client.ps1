...
function load-settings($s_file)
{
    ...
    $global:queueName = $settings.GrouperExchangeQueueName
    $global:retryQueueName = $settings.GrouperExchangeRetryQueueName
    $global:GrouperUser = $settings.GrouperUser
    $global:GrouperPassword = $settings.GrouperPassword
    $global:GroupsOU = $settings.ExchangeGroupsOU
...
# Process a Grouper Exchange ActiveMQ message.
# Exchange Distribution Groups are managed by Grouper. They all exist under a specific Grouper Stem.
# There are two types of Exchange DGs - "Hybrid", and "Migrated" or "Exchange". Hybrid groups contain
# a single member, which is an SFU maillist. "Migrated" or "Exchange" groups contain the members directly.
# 
# To process a Hybrid group:
#  - fetch the single maillist member
#  - fetch the members of that maillist
# Exchange DG in AD. Nested  
#
